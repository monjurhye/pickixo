...
test('অনুচ্ছেদ ১(৩)', 'the percentage applies to the increase, never to basic pay', () => {
  ...
  // অনুচ্ছেদ ১(৩)(গ) gives no figure for 1 July 2027; it is read as the full fixed pay and
  // tagged ASSUMPTION so the UI can say so.
  assert.equal(r.phases[2].payable, r.newBasic, 'phase 3 is the fixed 2026-scale pay');
  assert.equal(r.phases[2].certainty, 'ASSUMPTION');
  assert.equal(r.phases[2].addedToCurrentBasic, r.newBasic - 9300);
});

test('অনুচ্ছেদ ১(৩)', 'a grade 6 employee on the 4th step: 41,110 → 82,200 (57,546 / 69,873 / 82,200)', () => {
  ...
  // 1(৩)(গ) full pay → 82200 (read as the fixed pay; the Gazette gives no figure).
  ...
  assert.deepEqual(r.phases.map((p) => p.payable), [41110 + 16436, 41110 + 28763, 82200]);
});
